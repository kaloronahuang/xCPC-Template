...
const int Gi = fpow(G, mod - 2), inv2 = fpow(2, mod - 2);
...
void poly_sqrt(int deg, int *src, int *dst)
{
    static int tmp_sqrt[MAX_N], D[MAX_N];
    if (deg == 1)
        return (void)(dst[0] = 1);
    poly_sqrt((deg + 1) >> 1, src, dst);

    for (int i = 0; i <= (deg << 1); i++)
        D[i] = 0;
    poly_inverse(deg, dst, D);
    ...
    ntt(dst, 1, poly_siz), ntt(D, 1, poly_siz), ntt(tmp_sqrt, 1, poly_siz);
    for (int i = 0; i < poly_siz; i++)
        dst[i] = 1LL * inv2 * ((0LL + dst[i] + 1LL * tmp_sqrt[i] * D[i] % mod) % mod) % mod;
    ntt(dst, -1, poly_siz);
